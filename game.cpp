...
void checkAchieves() {
    ...
    for (unsigned int i = 0; i < achievesNumber; i++) {
        if (achievesPicked[i] && !achievesShowed[i]) {
            ...
            wait();
            if (i == 3) {
                drawModalWindow(pressRForCharity, 0, L"[R]");
                wait('r','R');
                }
...
void showMainMenu() {
    wstring s = days21;
    s+=mainMenuText;
    drawWindow(s, gameName, L"", txt(L"%ls    %ls", one_or_two, developer));
    wchar_t menu = getAnswer('1', '2');
...
wstring printExactTask(int course, int task, int word) {
    switch (course) {
        case 1:
        case 2:
            return wstring(programmingTasks[task]);
            break;
        case 3:
        case 4:
            return wstring(mobileTasks[task]);
            break;
        case 5:
            return wstring(webProgrammingTasks[task]);
            break;
        case 6:
            return wstring(systemProgrammingTask[task]);
            break;
        case 7:
            return wstring(adminTasks[task]);
            break;
        case 0:
        default:
...
void showLearnDialog() {
    ...
    for (unsigned short i = 0; i < coursesNumber; i++) {
        ...
        if (i == 2) {
            ...
            for (int j = 0; j < advancedProgrammingCourses; j++) {
                s+=txt(L"     %lc.%ls [%lc]", (wchar_t)('a'+j), advProgCourses[j], (advProgCoursesFinished[j])?'X':' ');
        ...
        s+=txt(L"%lc.%ls [%lc]", (wchar_t)('1'+i), courses[i], (coursesFinished[i])?'X':' ');
    ...
    drawWindow(s, coursesTitle ,L"", number_or_enter);
    unsigned wchar_t answer = 0;
    answer = getKey();

    if (answer == ENTER_KEY)
        return;

    else if (answer >='1' && answer <=(wchar_t)('0'+coursesNumber) && answer != '3') {
        answer -- ;
        answer -='0';
...
void startNewGame() {
    // Intro
    drawWindow(gameIntroTextPart1, introTitle, L"", enter, true);
    wait();
    drawWindow(gameIntroTextPart2, introTitle, L"", enter, true);
    wait();
    drawWindow(gameIntroTextPart3, introTitle, L"", enter, true);
    wait();
    // Goals
    drawWindow(gameIntroPlan, 0, L"", enter, true);
    wait();
    // Desktop
    drawWindow(L"", introDesktop, L"", enter);
    wait();
    // Top line
    drawWindow(introStatusLine,L"", getStatusLine(), enter);
    wait();
    // Keys
    drawWindow(L"", L"", L"", introAllowedKeys);
    wait('1');
    // Dialog window
...
bool isGameOver() {
    ...
    if (finishedCoursesCount >= 12 && finalCardsUnlocked >= 5 && gameTime <= 22.0 && allAchievesShowed) {
        ...
        s += gameOver;
        drawWindow(s, congratsMsg, L"", pressF);
        wait('f','F');
        showFinalStats();
        return true;
        }
    ...
    if (playerMoney < 0 && karma() < 0 && noPopularity && floor(unreadMessages) == 0 && gameTime > 22.0 &&
        ...
        wstring s = gameOverLogo;
        s += gameOverText;
        drawWindow(s, gameOverTitle, L"", pressF);
        wait('f','F');
        showFinalStats();
        return true;
        }
...
std::wstring getStatusLine() {
    wstring s = txt(statusLine, (int)gameTime, (int)playerMoney, karma(), (int)floor(unreadMessages));
    if (showCoursesTab)
        s += txt(statusLineCoursesNotFinished, getAvailableCoursesCount());
    return s;
    }

wstring getBottomLine() {
    wstring s;
    if (showCoursesTab)
        s += bottomLineCourses;
    s += bottomLineMsgAchieves;
    if (wchar_tityUnlocked) {
        s += charityTitle;
        s += L"[R]    ";
        }
...
int showUnreadMessages() {
    ...
    wstring s = printExactTask(newTaskLevel, task, word);
    s += txt(yourAnswer,levelYesAnswer[yes],levelNoAnswer[no]);
    wstring buttons = one_or_two;
...
bool messageGetAnswer(int level) {
    ...
    wchar_t answerKey = getAnswer('1', '2', ESCAPE_KEY);
...
double showChangeSpeedDialog(wstring e = L"") {
    wstring s;
    if (dt >= 1.0)
        s = txt(daysPerSecond, (int)dt);
    else {
        s = txt(daysPerSecond, 0);
        s += txt(L".%d", (int)(dt*10)%10); // Actually not the best way of printing double values
        }
    if (!e.empty())
        s += e;
    drawModalWindow(s.c_str(), changeSpeedTitle, changeSpeedButtons);
    wchar_t ch = getAnswer('+','-','Y','y');
    if (ch == 'y' || ch == 'Y')
        return dt;
    else if (ch == '+') {
        if (dt >= 1.0)
        ...
        if (dt >= 10.0) {
            dt = 10.0;
            return showChangeSpeedDialog(L" max");
            }
        ...
        if (dt <= 0.10) {
            dt = 0.10;
            return showChangeSpeedDialog(L" min");
            }
        return showChangeSpeedDialog();
        }
    return dt;
                ...
                switch (e->idata) {
                    ...
                    case LETTER_UNNAMEDSTUDIO_3: {
                        ...
                        if (needMoney > 50000)
                            ...
                            needMoney *= -1;
                        wstring s = txt(specialLetters[14], needMoney);
                    ...
                    case LETTER_KNOWLEDGE_QUESTION: {
                        ...
                        if (nextKnowledgeLetterIndex > 27) {
                            ...
                            wstring s = knowledgeCheck[nextKnowledgeLetterIndex];
                            ...
                            drawWindow(s, new_letter, getStatusLine(), one_or_two);
                            wchar_t ch = getAnswer('1', '2');//getch();
                            if ((ch - '0') == yes) {
...
void mainGameCycle() {
    ...
    while (1) {
        ...
        wstring newStatusLine = getStatusLine();
